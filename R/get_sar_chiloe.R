...
# objetivos: Códido de Obtenga Información SAR Chiloe
# fecha: 29-08-2023
...
folder_roi <-  "data/shapes/"
path_roi <- paste0(folder_roi, "Comunas_Chile.shp")
...
roi <-  roi %>%  filter(NOM_PROVIN == "CHILOÉ") %>% 
  make_box(d_buffer = 1000)
...
# save bpc sitio
path_out_box <- paste0("data/turberas/shapes/chiloe")
make_dir(path_out_box)
box_name <- paste0(path_out_box, "/chiloe_","_box_1000.shp")
st_write(roi, box_name, delete_dsn= T)
...
yearly_monthly_sum <- function(year){
  monthly_sum <- function(m){
    w <- VV$filter(ee$Filter$calendarRange(m, m, 'month'))$sum()
    
    return(w$set('month', m)$
             set('system:time_start', ee$Date$fromYMD(year, m, 1)))
  }
  return(months$map(ee_utils_pyfunc(monthly_sum)))
}

monthlyPrecip <- ee$ImageCollection$fromImages(
  year$map(ee_utils_pyfunc(yearly_monthly_sum))$flatten()
)


monthly_mean <- function(m){
...
meanMonthlyPrecipitation <- ee$ImageCollection$fromImages(
  months$map(ee_utils_pyfunc(monthly_mean))$flatten()
)

clip_ImageCollection <- function(image){
  return(image$clip(ee_roi))
}

meanMonthlyPrecipitation_clipped <- meanMonthlyPrecipitation$map(clip_ImageCollection)

listOfImages <- meanMonthlyPrecipitation_clipped$toList(meanMonthlyPrecipitation_clipped$size())

Map$centerObject(eeObject = ee_roi,zoom = 14) 
Map$addLayer(ee$Image(listOfImages$get(0)),
