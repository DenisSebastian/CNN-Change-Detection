...
get_min <- function(band, geometry = NULL){
  min_pixel <- ee$Number(band$reduceRegion(
...
get_max <- function(band, geometry = NULL){
  max_pixel <- ee$Number(
    band$reduceRegion(
...
mean_by_geometry <- function(image, geom, scale = 30){
  mean_geom <- image$reduceRegions(
...
geometryEE <- geomtry2ee <- function(region){
  r <- region %>%
    sf_as_ee()
  return(r)
}
...
# Visualización Mapas EE --------------------------------------------------

# Visualización 1 Banda
map_ee <- function(img, ee_roi, pal, zoom = 12, name = "img"){
  min_img <- get_min(img)$getInfo()
  max_img <- get_max(img)$getInfo()
  viz_temp <- list(min=min_img, max=max_img, palette=pal)
  
  Map$centerObject(eeObject = ee_roi, zoom = zoom) 
  m <- Map$addLayer(img,visParams =viz_temp, name = name)
  return(m)
}

# papa RBG

map_rgb_ee <- function(img, ee_roi, min =0, max = 0.4, zoom = 12, name = "RBG"){
  viz_rgb <- list(bands = c("B4", "B3", "B2"), min = min, max = max)
  Map$centerObject(eeObject = ee_roi, zoom = zoom) 
  rgb <- Map$addLayer(img,visParams =viz_rgb, name = name)
  return(rgb)
}
...
dl_imgSat <- function(roi,  n_region,  year = "2022", comunas,
  ...
  if(n_region %in% reg_g){
    print(paste0("La región R", n_region, " Se procesará por comuna por su extensión"))
    ...
    dataset = get_image(roi = roi, year = year, imageSat = imageSat,
                        band = band, md_init = '-01-01',md_end = '-12-31')
...
dl_image <- function(image_ee, roi, n_sat, year,  description_d, 
  ...
  if(isTRUE(simplify_roi)){
    ...
    roi_ee <- roi %>% rgee::sf_as_ee()
    region <- roi_ee$bounds()
  }
  
  print("up drive")
  img_dl <- ee_image_to_drive(
    image = image_ee,
    description = paste(n_sat, description_d, year, sep = "_"),
  ...
  img <- ee_drive_to_local(
    task = img_dl,
    metadata = TRUE,
    dsn =paste0(path_out,"/", n_sat,"_",description_d, "_", year, ".tif")
  )
  return(img)
}
...
NDVI_f <- function(image, b_nir = "B8", b_red = "B4"){
  ndvi = image$expression("(nir - red) / (nir + red)", 
                          list(nir =  image$select(b_nir),
